...
class AddHabitsDialogFragment: DialogFragment(){
    ...
    private fun setTagAnimation(tagChoice: String, animTagView: LottieAnimationView) {
        when(tagChoice) {
            "Lectures" -> animTagView.setAnimation(R.raw.videolesson)
            "Lab" -> animTagView.setAnimation(R.raw.lap)
            "Tutorial" -> animTagView.setAnimation(R.raw.report)
            "Default-Target" -> animTagView.setAnimation(R.raw.study)
            else -> {
                animTagView.setAnimation(R.raw.study)
            }
        }
    ...
    private fun insertDataIntoDb() {
        ...
        if (!TextUtils.isEmpty(habitTitle) && habitMaxDayString != "")
        {
            ...
            Toast.makeText(context, "Study created", Toast.LENGTH_SHORT).show()
